...
const Header = () => {
  ...
  return (
    <header className="bg-gray-700 text-white p-4 sticky top-0 shadow-xl z-20">
      <nav className={`container mx-auto justify-between items-center`}>
        <div className='text-2xl font-bold'>Kinnari Tamhane</div>
        <div className="md:hidden">
          <button onClick={toggleMenu} className="focus:outline-none float-right mb-5 mt-2 absolute top-[10px] right-[15px]">
        ...
        <div className={`md:flex md:space-x-4 ${isOpen ? 'block' : 'hidden'} md:block`}>
          <ul className='md:pt-2 pt-10 h-[100vh] md:h-auto md:flex grid place-content-center text-center gap-8'>
            ...
            <li><a href="#experience" onClick={toggleMenu }className="block px-2 py-1 hover:font-bold text-2xl md:text-lg">Experience</a></li>
